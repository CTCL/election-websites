...
class Contact_Form {
	...
	public static function validate_fields() {
		...
		$fullname = trim( wp_strip_all_tags( filter_input( INPUT_POST, 'fullname' ) ?? '' ) );
		$email    = filter_input( INPUT_POST, 'email', FILTER_VALIDATE_EMAIL );
		$topic    = filter_input( INPUT_POST, 'topic' ?? '' );
		$message  = trim( wp_strip_all_tags( filter_input( INPUT_POST, 'message' ) ?? '' ) );
	...
	public static function validate( $nonce ) {
		...
		if ( $recaptcha_enabled ) {
			$token = wp_strip_all_tags( filter_input( INPUT_POST, 'token' ) );
			if ( $token ) {
