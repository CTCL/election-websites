...
registerBlockType( CHILD_BLOCK, {
	...
	edit( props ) {
		...
		const ALLOWED_BLOCKS = wp.blocks
			...
			.filter(
				( blockName ) => ! DISALLOWED_BLOCKS.includes( blockName )
			);

		return (
			<div className="accordion-section-editor">
	...
	save( props ) {
		return (
			<div
				className={ `accordion-section-wrapper ${
					props.attributes.isNestedGroup ? 'subsection' : ''
				}` }
			>
				{ createElement(
					getHeaderTag( props.attributes ),
					{
						className: getHeaderClasses( props.attributes ),
					},
					getIconEl( props.attributes ),
...
registerBlockType( NESTED_PARENT_BLOCK, {
	...
	save() {
		return (
			<section className="accordion-group subsection">
				<InnerBlocks.Content />
			</section>
		);
	},
